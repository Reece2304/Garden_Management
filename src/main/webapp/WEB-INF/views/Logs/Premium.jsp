...
<body>
	...
	<nav>
		<a class="logout" href="/logout">logout</a> <a class="logout"
			href="/u/log/home">logs</a> <input type="checkbox" id="check"
			name="check"> <label for="check" class="checkbtn"> <i
			class="fas fa-bars"></i></label> <label class="logo"><a
			href="/Userloggedin"><img src="../../../images/horted-logo.png"
				alt="Horted Logo"></a></label>

		<ul class="text">
			<li><a href="/u/garden" id="garden"> Your garden</a></li>
			<li><a href="/u/account" id="account"> Your account</a></li>
		</ul>
		<label class="welcome"> <span class="username">${plant.name}</span>'s
			Logs
		</label>
	</nav>
	...
	<div class="container_login">
		...
		<div class="container2_login">
			<span class="Form_title"> ${plant.name}'s Log Entries <br />
			</span>
			<table>
				<c:if test="${empty LogList == 'false'}">
					...
					<tr>
						<c:forEach items="${LogList}" var="Entries">
							<td style="font-weight: 550; font-size: 24px;">
								${Entries.dateOfEntry}</td>
							<td style="font-weight: 550; font-size: 24px;">
								${Entries.pruning }</td>
							<td style="font-weight: 550; font-size: 24px;">
								${Entries.nutrition}</td>
							<td style="font-weight: 550; font-size: 24px;">
								${Entries.pesticide}</td>
						</c:forEach>
						
						<c:forEach items="${happyList}" var="Entries">
							<c:if test="${Entries.happy == 'true' }">
								<td><img src="../../../images/happy_plant.png"
									style="height: 75px; width: 75px;" /></td>
							</c:if>
							<c:if test="${Entries.happy == 'false' }">
								<td><img src="../../../images/sad_plant.png"
									style="height: 75px; width: 75px;" /></td>
							</c:if>
						</c:forEach>
					</tr>
				</c:if>
				<c:if test="${empty LogList == 'true'}">
					<tr>
						<th>Date</th>
						<th>Happiness</th>
					</tr>
